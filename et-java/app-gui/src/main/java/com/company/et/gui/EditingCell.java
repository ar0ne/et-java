                ...
                public void handle(Event event) {
                    ...
                    if (getTreeTableRow().getTreeItem().getParent().getChildren().size() > 1) {
                        ...
                        if (getTreeTableRow().getTreeItem().getParent().getValue().getProfessorsWork().equals(UCHEBNO_METOD)) {
                            mainSceneController.getCurrentProfessor().getTasks().get(0).remove(deletedTask.getValue());
                        } else if (getTreeTableRow().getTreeItem().getParent().getValue().getProfessorsWork().equals(NAUCHNAYA)) {
                            mainSceneController.getCurrentProfessor().getTasks().get(1).remove(deletedTask.getValue());
                        } else {
                            mainSceneController.getCurrentProfessor().getTasks().get(2).remove(deletedTask.getValue());
                        }
                    } else {
                        Dialogs.create()
                                .title("Ошибка")
